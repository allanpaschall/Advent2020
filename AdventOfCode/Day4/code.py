...
s = """eyr:2029 byr:1931 hcl:z cid:128
...
for passport in passports:
	newdict = {}
	listport = passport.split(' ')
	for field in listport:
		name = re.sub(':.*','',field)
		value = re.sub('.*:','',field)
		newdict[name] = value
	dictports.append(newdict)

#part1
def HaveAllReqFields(passports):
...
validports = HaveAllReqFields(s)

print('Part 1: \n',len(validports),'passports have all required fields.')

#format
yrstring = re.compile(r'^[\d\d\d\d]{4}$')
hcl = re.compile(r'^#[0-9A-Fa-f]{6}$')
pid = re.compile(r'^[0-9]{9}$')
cmcheck = re.compile(r'^[0-9]{3}$')
incheck = re.compile(r'^[0-9]{2}$')
...
def CheckYearFmt(inputstring):
	if yrstring.match(inputstring):
		return(True)
	else:
		return(False)

def CheckValidYear(inputyear,field):
	if CheckYearFmt(inputyear):
		if int(inputyear) >= field[0] and int(inputyear) <= field[1]:
			return(True)

def CheckValidHeight(inputstring):
	if 'cm' in inputstring:
		inputstring = inputstring[0:3]
		if cmcheck.match(inputstring):
			if int(inputstring) >= hgtc[0] and int(inputstring) <= hgtc[1]:
				return(True)
	else:
		inputstring = inputstring[0:2]
		if incheck.match(inputstring):
			if int(inputstring) >= hgti[0] and int(inputstring) <= hgti[1]:
				return(True)
	return(False)

def CheckValidHairColor(haircolor):
	if hcl.match(haircolor):
		return(True)
	else:
		return(False)

def CheckValidEyeColor(eyecolor):
	if eyecolor in ecl:
		return(True)
	else:
		return(False)

def CheckValidPID(passportidentificationnumber):
	if pid.match(passportidentificationnumber):
		return(True)
	else:
		return(False)

def CheckPassport(passport):
	if not CheckValidPID(passport['pid']):
		print("Invalid PID")
		return(False)
	if not CheckValidYear(passport['byr'], byr):
		print("Invalid Birth Year")
		return(False)
	if not CheckValidYear(passport['iyr'], iyr):
		print("Invalid Issued Year")
		return(False)
	if not CheckValidYear(passport['eyr'], eyr):
		print("Invalid Expiration Year")
		return(False)
	if not CheckValidHeight(passport['hgt']):
		print("Invalid Height: ", passport['hgt'])
		return(False)
	if not CheckValidHairColor(passport['hcl']):
		print("Invalid Hair Color")
		return(False)
	if not CheckValidEyeColor(passport['ecl']):
